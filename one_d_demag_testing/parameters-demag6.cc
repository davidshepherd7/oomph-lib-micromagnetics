// Parameters for M = sin(2*pi*x) * [cos(omega*t), sin(omega*t), 0] (M=0 at ends)
...
namespace OneDMicromagSetup
{
  ...
  {
    return -2.0*cos(2*Pi*x[0])*cos(omega*t);
  }
  ...
  void llg_source_function(const double& t, const Vector<double>& x, Vector<double>& source)
  {
    ...
    source[0] = 4*Pi*sin(2*Pi*x[0])*sin(2*Pi*x[0])*sin(2*Pi*x[0])*cos(omega*t)*sin(omega*t)*sin(omega*t) 
      - omega*sin(2*Pi*x[0])*sin(omega*t);
    source[1] =  omega*sin(2*Pi*x[0])*cos(omega*t) 
      - 4*Pi*sin(2*Pi*x[0])*sin(2*Pi*x[0])*sin(2*Pi*x[0])*cos(omega*t)*cos(omega*t)*sin(omega*t);
    source[2] = 4*Pi*sin(2*Pi*x[0])*sin(2*Pi*x[0])*cos(omega*t)*sin(omega*t);
  }
