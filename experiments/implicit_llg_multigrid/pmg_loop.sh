...
for smoother in $smoothers; do
    ...
    # echo "nx,dt,smoother,vc,avg_newton_its,avg_solver_its" > iteration_counts_$smoother

    #for nx in $nxs; do
        for dt in $dts; do
            for vc in $v_cycles; do
                LABEL="nx$1_dt${dt}_smth${smoother}_vc_$vc"
                DIR="${BASE}/${LABEL}"
                echo $LABEL
                mkdir -p $DIR
                ./multigrid_experiments '-nx' $1 '-ny' $1 '-dt' $dt '-tmax' $dt \
