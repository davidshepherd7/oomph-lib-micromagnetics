...
for nx in $nxs; do
    for dt in $dts; do
        DIR="${SBASE}/nx${nx}_dt${dt}"
        echo $DIR
        mkdir -p $DIR

        ./multigrid_experiments '-nx' $nx '-ny' $nx '-dt' $dt \
            '-tmax' $dt  '-outdir' $DIR > ${DIR}/trace
        mv "${DIR}/jacobian_tmax" "${DIR}/jacobian_tmax_nx${nx}_dt${dt}"
    done
done
...
for refine in $refines; do
    for dt in $dts; do
        DIR="${UBASE}/refine${refine}_dt${dt}"
        echo $DIR
        mkdir -p $DIR
        ./multigrid_experiments '-r' $refine '-dt' $dt '-tmax' $dt \
            '-outdir' $DIR > ${DIR}/trace
